...
$(document).ready(function(){
    ...
    $('#form_referencias').submit(function(event){
        ...
        if ($('#notes').val() === "") {
            alert('Debe ingresar las notas','Atencion!');
            $('#notes').focus();
            return false;
        }
    ...
    $('#form_assign_referencias').submit(function(event){
      if ($('#assign_id_reference').val() === "") {
          alert('Debe ingresar el id que desea al que desea que sea asociado','Atencion!');
      ...
      let confirmacion=confirm("¿Esta seguro de asignar a este id?");
        if(confirmacion){
          ...
          let new_reference_id= document.getElementById("assign_id_reference").value;
          var data = [ID,new_reference_id];
          $.ajax({
            url: $('#form_assign_referencias #_url').val()+"/assign/"+data,
            headers: {'X-CSRF-TOKEN': $('#form_assign_referencias #_token').val()},
            type: 'GET',
            cache: false,
            data: data,
            success: function (response) {
